...
```bash
git -C workspace show {commit_sha}:path/to/file
```
...
```sql
-- Replace YOUR_METRIC with the actual metric column name
SELECT trial_id, commit_sha, status, YOUR_METRIC
FROM trials WHERE status = 'success'
ORDER BY YOUR_METRIC DESC;
```
